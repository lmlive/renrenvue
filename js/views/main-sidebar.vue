...
<script>
    ...
    define(['vue', 'v!views/main-sidebar-sub-menu', 'validate'], function (Vue, SubMenu, validate) {
        var isURL = validate.isURL
        return Vue.component('main-sidebar',{
            template: template,
            data() {
                return {
                    dynamicMenuRoutes: []
                    ...
                    if (route.meta.isTab) {
                        ...
                        if (!tab) {
                            if (route.meta.isDynamic) {
                                route = this.dynamicMenuRoutes.filter(item => item.name === route.name)[
                                    0]
                                if (!route) {
